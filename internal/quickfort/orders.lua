...
local function inc_order_spec(order_specs, quantity, reactions, label)
    ...
    if not order_specs[label] then
        ...
        for _,v in ipairs(reactions) do
            local name = dfhack.lowerCp437(v.name)
            -- just find the first procedurally generated buildable instrument
            if label == 'instrument' and name:find('^assemble [^ ]+$') then
                local iname = name:match('^assemble (.*)')
                for _, instrument in ipairs(df.global.world.raws.itemdefs.instruments) do
                    if instrument.source_enid == df.global.plotinfo.civ_id and
                        instrument.flags.PLACED_AS_BUILDING and
                        dfhack.lowerCp437(instrument.name) == iname
                    then
        ...
        if not order then
            if label == 'instrument' then
                -- no buildable instruments found; this is a quirk of the generated
                -- world and is not an error
                return
            else
                error(string.format('unhandled label: %s', label))
            end
        end
